...
let rt_obj = 0;
let rt_amb = 0;
let obj_list: Array<number> = [];
let amb_list: Array<number> = [];

class Process extends Draw_Canvas {
    canvas: HTMLCanvasElement;
    recv_buf: Array<number> = [];

    obj_text_div: HTMLElement;
    amb_text_div: HTMLElement;


    sample_count: number = 0;

    package_size: number = 64;
    recv_total_pack_count: number = 0;
    recv_data_pack_count: number = 0;
    ...
    draw_chart_0() {
        ...
        var t_head;
        if (this.recv_data_pack_count < 1) { return }
        ...
        if (this.recv_data_pack_count < chart_max_len) {
            ...
            if (sc == sc1) {
                if (sc >= chart_max_len) { t_head = sc - chart_max_len; } else { t_head = 512 - chart_max_len + sc; }
    ...
    rcv_data_input(d) {
        var kk: Array<number> = Array.prototype.slice.call(new Uint8Array(d.buffer));
    ...
    rcv_data_process() {
        ...
        if (len < 16) return 0;//this.package_size
